...
public class TicTacToeBoard {
    private char[][] grid = new char[3][3]; //questionable, originally have final
    ...
    public boolean move(String input, char mark) throws InvalidInputException,
        ...
        if(input.length()!= 2 || (!Character.isLetterOrDigit(input.charAt(1))) || (!Character.isLetterOrDigit(input.charAt(0)))){
            throw new InvalidInputException();
        }
        if( !(input.charAt(0)<= 'C' && input.charAt(0)>= 'A')){
            throw new InvalidInputException();
        }
        if( !(input.charAt(1)<= '3' && input.charAt(1)>= '1')){
            throw new InvalidInputException();
        }
        /** 
    ...
    public boolean move(int row, int col, char mark) 
         ...
         */
        if(getSpace(row, col) == 'O' ||  getSpace(row, col)== 'X'){
