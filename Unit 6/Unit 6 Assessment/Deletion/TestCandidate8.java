 ...
 * Candidate tester class using ArrayLists and replacing elements and inserting elements and deleting elements
 * @author (Jeffrey Chiu) 
 * @version (06/04/18)
 */
public class TestCandidate8
{
    ...
    public static List<Candidate> deleteByLoc(List<Candidate> c1, int loc){
        c1.remove(loc);
        return c1;
    } 
    public static List<Candidate> deleteByName(List<Candidate> c1, String name){
        int loc = -1;
        for(Candidate c : c1){
            if(c.getName().equals(name)){
                loc = c1.indexOf(c);
