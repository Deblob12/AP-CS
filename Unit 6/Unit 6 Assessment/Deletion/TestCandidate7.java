 ...
 * Candidate tester class using arrays and replacing elements and inserting elements and deleting elements
 * @author (Jeffrey Chiu) 
 * @version (06/05/18)
 */
public class TestCandidate7
{
    ...
    public static Candidate[] deleteByLoc(Candidate[]c1, int loc){
        Candidate[] c2 = new Candidate[c1.length - 1];
        for(int i = 0; i < loc; i++){
            c2[i] = c1[i];
        }
        for(int j = loc; j < c2.length; j++){
            c2[j] = c1[j + 1];
        }
        return c2;
    }
    public static Candidate[] deleteByName(Candidate[]c1, String name){
        int loc = -1;
        for(int i = 0; i < c1.length; i++){
